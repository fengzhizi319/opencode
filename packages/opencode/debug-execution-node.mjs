...
`);
  ...
  // ========== 阶段 1: 初始化 ==========
  // 目的：初始化项目实例，设置工作目录和环境
  DEBUG.stage(1, "项目初始化");
  
  DEBUG.log("正在初始化项目实例...");
  // 在实际运行中，这里会调用 Instance.initialize() 来加载项目配置
  ...
  // 模拟会话数据（实际运行时会通过 Session.create() 创建）
  const mockSession = {
    id: "01HQ8V8X1Y2Z3W4V5U6T7S8R9Q",  // 会话唯一标识符
  ...
  // 模拟可用的 Skills（实际会通过 Skill.available() 获取）
  const mockSkills = [
  ...
  // 模拟可用工具（实际会通过 ToolRegistry.tools() 获取）
  const mockTools = [
  ...
  events2.forEach((e, i) => {
    console.log(`${i + 1}. [${e.type}] ${e.desc}`);
    if (e.isContent) {
      console.log(`   内容: "${e.desc.substring(0, 60)}..."`);  // 截断长文本
    }
    if (e.detail) {
